...
const supaUrl=localStorage.getItem(STORAGE_URL);
const supaKey=localStorage.getItem(STORAGE_KEY);

if(!supaUrl||!supaKey){showSetup();return;}
...
function showSetup(){
  ...
  o.innerHTML='<div style="background:#1a1208;border:1px solid #2e7d52;border-radius:10px;padding:32px;max-width:380px;width:90%;color:#f4efe2;font-family:sans-serif;"><div style="font-size:1.1rem;margin-bottom:8px;font-style:italic;">Welcome to Hyph\u00e6lium</div><div style="font-size:0.82rem;color:#9a8e7a;margin-bottom:20px;">Enter your Supabase credentials to connect your vault.</div><div style="margin-bottom:10px;"><div style="font-size:0.65rem;text-transform:uppercase;color:#5a7a5a;margin-bottom:4px;">Supabase Project URL</div><input id="hypha-url-in" type="text" placeholder="https://xxxxx.supabase.co" style="width:100%;background:#0e0b06;border:1px solid #2e7d52;border-radius:4px;padding:8px;color:#f4efe2;font-size:0.82rem;box-sizing:border-box;outline:none;"></div><div style="margin-bottom:18px;"><div style="font-size:0.65rem;text-transform:uppercase;color:#5a7a5a;margin-bottom:4px;">API Key</div><input id="hypha-key-in" type="password" placeholder="sb_publishable_... or eyJ..." style="width:100%;background:#0e0b06;border:1px solid #2e7d52;border-radius:4px;padding:8px;color:#f4efe2;font-size:0.82rem;box-sizing:border-box;outline:none;"></div><div style="display:flex;gap:8px;"><button id="hypha-save-btn" style="flex:1;background:#2e7d52;color:#fff;border:none;border-radius:4px;padding:10px;cursor:pointer;">Connect \u2192</button><button id="hypha-cancel-btn" style="background:transparent;border:1px solid #2e7d52;color:#4caf80;border-radius:4px;padding:10px 14px;cursor:pointer;">Cancel</button></div></div>';
  document.body.appendChild(o);
  document.getElementById('hypha-url-in').focus();
  document.getElementById('hypha-save-btn').onclick=()=>{
    const u=document.getElementById('hypha-url-in').value.trim().replace(/\/+$/,'');
    const k=document.getElementById('hypha-key-in').value.trim();
    if(!u||!k)return;
